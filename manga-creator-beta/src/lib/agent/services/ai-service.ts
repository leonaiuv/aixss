...
只输出 JSON，不要包含其他内容。`;
    ...
    if (!parsed?.scenes || !Array.isArray(parsed.scenes)) {
      return { success: false, error: "解析 AI 响应失败：缺少 scenes 字段" };
    }
...
只输出 JSON，不要包含其他内容。`;
    ...
    if (!parsed?.sceneDescription || !parsed?.keyframePrompt || !parsed?.spatialPrompt) {
      return { success: false, error: "解析 AI 响应失败：缺少必要字段" };
    }
...
export async function batchRefineWithAI(
  ...
  if (results.length === 0) {
    return { success: false, error: `所有分镜细化失败：${errors.join("; ")}` };
  }
...
export function formatExportData(data: ExportData, format: "json" | "txt" | "csv"): string {
  switch (format) {
    case "json":
    ...
    case "txt": {
      ...
      for (const scene of data.scenes) {
        ...
        if (scene.sceneDescription) output += `场景描述：${scene.sceneDescription}\n`;
        if (scene.fullPrompt) output += `完整提示词：${scene.fullPrompt}\n`;
        if (scene.spatialPrompt) output += `时空提示词：${scene.spatialPrompt}\n`;
        output += "\n";
      }
      return output;
    }
